...
V_PANIM = "varPAnim90"       # 주인공 좌우 방향 0=idle 1=우 2=좌
...
def build_player_blocks():
    ...
    # 좌우 방향 코드 → 코스튬 (1 right / 2 left). 상하는 마지막 좌우 유지
    sw_r = b_switch_costume(bs, "right")
    if_r = b_if(bs, cmp_op("operator_equals", vrep("이동애니", V_PANIM), 1), sw_r)
    sw_l = b_switch_costume(bs, "left")
...
def main():
    ...
    # 주인공 좌/우 스프라이트만 (상하는 이동만, 고개 돌리지 않음)
    c_pl_dirs = {
        "right": costume_png("player_right", PLAYER_SVG),
        "left": costume_png("player_left", PLAYER_SVG),
    }
    c_pl = c_pl_dirs["right"]
  ...
  <circle cx="2" cy="2" r="1" fill="#FF0000" opacity="0.01"/>
    ...
    player = {
        ...
        "variables": {V_PANIM: ["이동애니", 0]}, "lists": {}, "broadcasts": {},
        "blocks": player_blocks, "comments": {},
        "currentCostume": 0,
        "costumes": [
            {**c_pl_dirs["right"], "name": "right"},
            {**c_pl_dirs["left"], "name": "left"},
        ],
    ...
    project = {
        ...
        "meta": {"semver": "3.0.0", "vm": "13.7.4-svg", "agent": "robot-shooter-v8"}
    }
