...
| [체스 대전쟁](games/chess-war/) | 액션 — 라인 배틀 | 팔라독/냥코 대전쟁 스타일. 하얀 킹 vs 검은 킹, 하단 버튼으로 폰·비숍·나이트·룩·퀸 전사를 소환하면 알아서 전진해 자동 전투. 골드는 초당 자동 축적, 검은 킹을 체크메이트하면 강화 택1로 다음 스테이지. 적은 스테이지마다 지수(곱하기)로 세집니다. **85개 한글 손잡이 + 가성비(공격력÷코스트) 저울질로 코드를 바꾸며 놀 수 있습니다.** |
