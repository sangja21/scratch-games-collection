...
| 게임 | 메커닉 | 난이도 | 학습 포인트 |
|------|--------|-------|------------|
| **우주선 표류 (Asteroids)** | 분사 버튼이 가속도 벡터를 더함. 관성으로 계속 미끄러짐 | ★★★ | 속도 = 벡터 누적, 위치 = ∫속도 |
| **벡터 합 퍼즐** | 화살표 카드(예: →3, ↑2)를 골라 합성해 보물 지점 도달 | ★★★ | `v₁ + v₂` 시각화 |
| **미사일 요격** | 적 미사일의 위치+속도 벡터를 보고 교차점 예측해 발사 | ★★★★ | 상대속도, 외삽 |
| **돛단배 항해** | 바람 벡터 + 돛 각도로 합성 → 항구까지 항해 | ★★★ | 사영(projection), 합성 |
| **줄다리기 합력** | 3~4 방향에서 끄는 힘 벡터 합 → 결과 방향으로 깃발 이동. 균형 맞추기 | ★★★ | 평형 = ΣF = 0 |
| **농구 슛 라인** | 각도 · 힘 슬라이더로 발사 벡터 결정, 골대 통과 | ★★★ | 분해 (수평/수직 성분) |
| **도킹 미션** | 적 우주선과 같은 속도 벡터로 맞춘 뒤 결합 | ★★★★ | 상대속도 0 만들기 |

**👉 추천 묶음**: `벡터 합 퍼즐` (개념 입문) → `우주선 표류` (액션 응용) → `미사일 요격` (도전)

### 🌊 사인 / 코사인

| 게임 | 메커닉 | 난이도 | 학습 포인트 |
|------|--------|-------|------------|
| **삼각함수 서핑** | `y = A·sin(B·x + C)` 파도 위를 서퍼가 글라이드, 장애물 회피. `polynomial-shooter` 골격 재사용 가능 | ★★★ | 진폭 / 주기 / 위상 |
| **파형 매칭** | 화면에 회색 목표 파형. A/B/C 슬라이더로 빨간 곡선을 겹치게 맞추기 | ★★★ | 파라미터 의미 직관화 |
| **심전도 진단** | 환자 심박을 사인파 합성으로 보여줌. 비정상 파형(높은 진폭/짧은 주기) 찾기 | ★★★ | 의학적 응용 |
| **음정 튜너** | 표준음 440Hz에 주파수 슬라이더로 맞춤. 비트(맥놀이)가 사라질 때 정답 | ★★ | 주파수 = 1/주기 |
| **진자 박자 게임** | 사인 운동하는 진자가 양 끝점 도달할 때 클릭 (리듬) | ★★★ | sin 의 극값 = 주기/4 지점 |
| **푸리에 합성** | 사인파 2~3 개를 합쳐 목표 톱니파/사각파 만들기. 진폭 슬라이더 | ★★★★ | 푸리에 입문 |
| **등대 신호** | 등대가 회전하며 빛이 (cos θ, sin θ) 따라 돈다. 빛 닿을 때 배가 클릭 응답 | ★★★ | 단위원 좌표 |

**👉 추천 묶음**: `파형 매칭` (감각 잡기) → `삼각함수 서핑` (액션) → `등대 신호` (단위원 확장)
